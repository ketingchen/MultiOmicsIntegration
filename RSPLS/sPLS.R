...
permute=function(splsObj, B=10000, nThreads=1, p.adjust=T){
  ...
  if(p.adjust==T) pval=p.adjust(pval, method="BH")
  genes=cbind(importance=vips0, pvalue=pval)
  
  return(invisible(genes))
}

tune.spls=function(X, Y, lv=c(1 ,5, 10), lamda1=c(0.2, 0.4, 0.6), plot=F, cv.fold=4, cv.repeats=1, nThreads=1, verbose=1,...){
  ...
  tune.cv=function(X, Y, lv, lamda1, verbose, nThreads){
    ...
    for(j in 1:cv.fold){
      ...
      mod=sPLS(X=Xtrain, Y=Ytrain, lv=lv, lamda1=lamda1, verbose=F, quality.check=F)
  ...
  for(i in 1:nrow(tuned)){
    ...
    for(r in 1:cv.repeats){
      cat("   CV repeats:", r)
      # tune.cv=function(X, Y, lv, lamda1, verbose, nThreads){
      cv.result=tune.cv(X=X, Y=Y, lv=tuned[i,"lv"], lamda1=tuned[i,"lamda1"], verbose=1, nThreads=nThreads)
      r2[r,]=cv.result[,"r2"]
